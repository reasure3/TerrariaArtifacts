...
class SendShowInfoDataPacket(val data: ShowInfoData) : CustomPacketPayload {
    ...
    fun encode(buffer: ByteBuf) {
        data.run {
            ...
            buffer.writeBoolean(showPosition)
            ...
            buffer.writeBoolean(showMovementSpeed)
            buffer.writeBoolean(showTreasure)
            buffer.writeBoolean(showRareCreature)
            buffer.writeBoolean(showDps)
        }
    }
